...
pub fn render_system(
    ...
    for (entity, text_line, inner) in q_editable_texts.iter() {
        ...
        if let Some(cursor_pos) = text_line.cursor_position {
            ...
            cursor_fake_text.0 = text_line.get_text_range((CharPosition(0), cursor_pos)).unwrap();
        ...
        if let Some((selection_start, selection_end)) = text_line.selection_range() {
            let Ok(mut fake_text_before_selection) = q_texts.get_mut(inner.fake_text_before_selection) else {
                continue;
            };
            ...
            commands.entity(inner.fake_selection_text).insert(Visibility::Visible);
            info!("Rendered selection with text \"{}\"", fake_selection_text.0);
        } else {
            commands.entity(inner.fake_selection_text).insert(Visibility::Hidden);
        }
    }
}
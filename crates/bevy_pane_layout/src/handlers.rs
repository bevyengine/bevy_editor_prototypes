...
#[allow(unused)]
pub(crate) fn on_pane_drag_drop(
    trigger: Trigger<Pointer<DragDrop>>,
    mut commands: Commands,
    input: Res<ButtonInput<KeyCode>>,
    pane_query: Query<(), With<PaneRootNode>>,
    // divider_query: Query<&divider>,
    mut size_query: Query<&mut Size>,
    children_query: Query<&Children>,
    parent_query: Query<&Parent>,
    ...
    if target == dropped
        || !pane_query.contains(dropped)
        || trigger.event().button != PointerButton::Primary
    {
        return;
    }

    let vertical = input.pressed(KeyCode::ShiftLeft);

    println!("On drop.");

    // TODO Pick how we divider based on where the pane was dropped.
    // Currently dropping a pane will only add it to the existing divider
    // after the target pane it was dropped on top of.

    let previous_parent = parent_query.get(dropped).unwrap().get();
    let new_parent = parent_query.get(target).unwrap().get();

    let old_size = size_query.get(dropped).unwrap().0;
    let target_size = size_query.get(target).unwrap().0;

    // Are we staying on the same parent
    let same_parent = previous_parent == new_parent;

    let new_size = if same_parent {
        old_size
    } else {
        target_size / 0.
    };
